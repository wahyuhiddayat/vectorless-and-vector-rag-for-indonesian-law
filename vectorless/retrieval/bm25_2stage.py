"""
BM25 retrieval strategy for Indonesian legal QA.

All retrieval decisions (doc selection, node selection) use BM25 keyword scoring.
No LLM in retrieval — only for final answer generation.

This serves as the algorithmic baseline: pure keyword matching on legal text,
grounded in the finding that BM25 outperforms semantic search for formal
...
def retrieve(query: str, top_k_docs: int = 3, top_k_nodes: int = 5,
    ...
    if verbose:
        print(f"{'='*60}")
        print(f"Query: {query}")
        print(f"Strategy: bm25 (top_k_docs={top_k_docs}, top_k_nodes={top_k_nodes})")
        print(f"{'='*60}")
    ...
    if not doc_ids:
        return {"query": query, "strategy": "bm25", "error": "No relevant documents found"}
    ...
    if not node_ids:
        return {"query": query, "strategy": "bm25", "doc_ids": doc_ids,
