...
def _clean_text(text: str, usage_acc: dict, lock: threading.Lock) -> tuple[str, list[dict], str | None]:
    """Return (cleaned_text, fixes_list, rejection_reason). Original echoed back if rejected."""
    if not text.strip():
        return text, [], None
...
def clean_doc(doc_id: str, force: bool = False, verbose: bool = True) -> dict:
    ...
    counter["skipped"] = sum(1 for _ in _iter_leaves(doc.get("structure", []))) - len(todo)
