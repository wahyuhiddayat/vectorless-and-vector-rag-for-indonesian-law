"""
build_pasal.py — Build Pasal-level structural index from scraped Indonesian legal PDFs.

Reads registry.json + metadata from scraper, parses each PDF into a
hierarchical tree, enriches with metadata, and saves to data/index_pasal/.
Leaf nodes are at Pasal level (no Ayat/Huruf/Angka splitting).

Usage:
    python -m vectorless.indexing.build_pasal                          # index all documents (with LLM cleanup)
    python -m vectorless.indexing.build_pasal --no-llm                 # skip LLM cleanup (dev/testing only)
    python -m vectorless.indexing.build_pasal --doc-id uu-20-2025      # index single document
    python -m vectorless.indexing.build_pasal --force                  # re-index even if exists
"""
...
DATA_INDEX = Path("data/index_pasal")
...
def enrich_doc(parse_result: dict, registry_entry: dict, metadata: dict | None) -> dict:
    ...
    The final document is what gets saved to data/index_pasal/{doc_id}.json and contains:
...
def main():
    ...
    Flow per document:
      ...
      -> add navigation_paths -> merge metadata -> save data/index_pasal/{doc_id}.json
